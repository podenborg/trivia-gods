...
export const api = {  
  fetchSessionData: async (existingToken: string|undefined, sessionId: string|undefined): Promise<{questions: IQuestion[], token: string}> => {
    try {
      const res = await axios(`${API_URL}/review-session?amount=${AMOUNT}${existingToken ? `&token=${existingToken}&sessionId=${sessionId}` : ""}`);      
      const { questions, token } = res.data.data;
